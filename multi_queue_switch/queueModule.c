 ...
 * @param q_ptr points to this structure
 * */
void term_queue(struct queue *q_ptr){
...
int write_queue(struct finsFrame *fins_in, struct queue *q_ptr){
	...
	if (q_ptr->num_elements==0){
		...
		node->next = q_ptr->tail;  //new fins frame read into the tail
		q_ptr->tail = node;
...
int read_queue(struct finsFrame *fins_in, struct queue *q_ptr){
	...
	if (q_ptr->num_elements>0){// it needs to contain more than one element to be read
