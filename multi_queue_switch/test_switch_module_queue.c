 ...
 * data (test_data) to the IP module and ARP modules*/
...
The switch needs to send a reply request based on the search from its table. This side acts as a given module
which is accessing the switch and sending query requests to it and updating its local cache based on the replies. It also offers
the capability to run through the local cache and determine the list of destinations based on VCI and direction flag.
Let us choose another module this time.
 */
